...
-- 1. Cấu hình tính năng Chống Treo Máy (Anti-AFK) tránh bị văng khỏi server
...
-- 2. Khởi tạo trạng thái Sea mặc định là false
_G.Sea1 = false
_G.Sea2 = false
_G.Sea3 = false

-- 3. Kiểm tra chính xác ID Map để kích hoạt Sea tương ứng (Gồm cả Server phụ/Event)
...
Tabs.Info:AddButton({
        ...
        Description="Discord",
        Callback=function()
            setclipboard(tostring("https://discord.gg/MjGt2bTWBS")) 
        end
})
Tabs.Info:AddButton({
    Title="dio_plzzz",
    ...
    while wait() do
        pcall(function()
            ...
            elseif string.len(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner"))==86 then
                ...
                Mob_Kill_Cake_Prince:SetDesc("Cake Prince : Spawn ✅")
            end
        end)
    end
end)
...
if Sea3 then
    local AutoMirageIsland = Tabs.Sea:AddSection("Auto Mirage Island")
...
local ToggleFindLeviathan = Tabs.Sea:AddToggle("ToggleFindLeviathan", {
    Title = "Auto Find Leviathan", 
    Description = "", 
    Default = false
})

ToggleFindLeviathan:OnChanged(function(Value)
    _G.AutoLeviathan = Value
end)
Options.ToggleFindLeviathan:SetValue(false)

-- Logic Auto Tìm đảo Leviathan
local LeviathanFindBoat = nil
spawn(function()
    while wait(1) do
        pcall(function()
            if _G.AutoLeviathan then
                ...
                -- Tìm vị trí đảo Frozen Dimension (Leviathan) trong Locations của WorldOrigin
                local LeviathanLoc = workspace._WorldOrigin.Locations:FindFirstChild("Frozen Dimension", true) or workspace._WorldOrigin.Locations:FindFirstChild("Leviathan", true)
                
                if not LeviathanLoc then
                    -- CHƯA CÓ ĐẢO: Kiểm tra thuyền hiện tại của bạn
                    local MyBoat = CheckBoat()
                    
                    if not MyBoat then
                        -- Không có thuyền -> Bay về bến thuyền Sea 3 để mua thuyền
                        local BoatPos = CFrame.new(-16927.451, 9.086, 433.864)
                        Tween(BoatPos)
                        
                        -- Đến gần bến thuyền thì tự động gửi lệnh mua thuyền
                        if (BoatPos.Position - hrp.Position).Magnitude <= 15 then
                            ReplicatedStorage.Remotes.CommF_:InvokeServer("BuyBoat", _G.SelectedBoat or "Guardian")
                        end
                        return
                    end
                    
                    -- Có thuyền rồi nhưng nhân vật chưa ngồi vào ghế lái
                    if char.Humanoid.Sit == false and MyBoat:FindFirstChild("VehicleSeat") then
                        local SeatPos = MyBoat.VehicleSeat.CFrame * CFrame.new(0, 1, 0)
                        Tween(SeatPos)
                        return
                    end
                    
                    -- Tọa độ hướng thẳng ra vùng biển sâu (Vùng đá / Vùng nguy hiểm cấp 6) để tìm đảo
                    LeviathanFindBoat = CFrame.new(-10000000, 31, 37016.25)
                    
                    -- Kiểm tra xem có quái biển nguy hiểm xuất hiện không (Né quái nếu có hàm check của bạn)
                    local hasEnemies = false
                    if type(CheckEnemiesBoat) == "function" and CheckEnemiesBoat() then hasEnemies = true end
                    if type(CheckTerrorShark) == "function" and CheckTerrorShark() then hasEnemies = true end
                    if type(CheckPirateGrandBrigade) == "function" and CheckPirateGrandBrigade() then hasEnemies = true end

                    if hasEnemies then
                        -- Nếu gặp quái, tự động bay vút lên cao (Cao độ 150) để bảo toàn mạng sống và giữ thuyền
                        Tween(CFrame.new(hrp.Position.X, 150, hrp.Position.Z))
                    else
                        -- Biển an toàn thì tiếp tục di chuyển thuyền tiến ra vùng biển sâu
                        Tween(LeviathanFindBoat)
                    end
                else
                    -- ĐÃ THẤY ĐẢO LEVIATHAN: Tự động rời ghế lái thuyền và bay thẳng lên đảo
                    local TeleportPart = LeviathanLoc:FindFirstChild("HeadTeleport", true) or LeviathanLoc:FindFirstChild("Teleport_Head", true) or LeviathanLoc:FindFirstChild("Head", true)
                    ...
                    if TeleportPart then
                        ...
                        -- Phương án dự phòng: Lấy thẳng vị trí trung tâm của đảo (Model/Part) để bay tới
                        local CenterPos = LeviathanLoc:IsA("Model") and LeviathanLoc:GetPivot().Position or LeviathanLoc.Position
                        local DirToIsland = (CenterPos - hrp.Position).Unit
                        local SafePos = CenterPos - DirToIsland * 100 + Vector3.new(0, 50, 0)
...
local ToggleDracoV4 = Tabs.Sea:AddToggle("ToggleDracoV4", {
    Title = "Auto Trial Draco V4", 
    Description = "", 
    Default = false
})
ToggleDracoV4:OnChanged(function(Value)
    _G.Relic123 = Value
end)
Options.ToggleDracoV4:SetValue(false)

-- Logic Auto Trial Draco V4
spawn(function()
    while wait(1) do -- Thay đổi 'Sec' thành vòng lặp 1 giây ổn định
        if _G.Relic123 then
            pcall(function()
                -- Lấy HumanoidRootPart từ biến có sẵn của source gốc
                local hrp = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart")
                if not hrp then return end

                -- Kiểm tra xem phòng DracoTrial đã được mở ra chưa
                if workspace.Map:FindFirstChild("DracoTrial") then
                    -- Gửi tín hiệu Remote lên Server để kích hoạt/tham gia
                    ReplicatedStorage.Remotes["DracoTrial"]:InvokeServer()
                    wait(0.5)
                    
                    -- Danh sách các tọa độ điểm Relic cần dịch chuyển qua trong phòng Trial
                    local RelicSpots = {
                    ...
                    -- Lặp qua từng tọa độ và dùng hàm Tween dịch chuyển nhân vật tới
                    for _, spot in ipairs(RelicSpots) do
                        repeat
                            wait()
                            Tween(spot) -- Thay thế '_tp' chưa định nghĩa bằng hàm 'Tween' của bạn
                        until not _G.Relic123 or (spot.Position - hrp.Position).Magnitude <= 10 -- Thay 'Root' bằng 'hrp'
                        wait(2.5) -- Đợi 2.5 giây tại mỗi điểm để Server xử lý ghi nhận cơ chế
                    end
                else
                    -- Nếu chưa mở phòng DracoTrial, tự động bay tới vị trí cổng Trial trên đảo Prehistoric Island
                    ...
                    if PrehistoricIsland then
                        ...
                        if TrialTeleport and TrialTeleport:IsA("Part") then
                            Tween(CFrame.new(TrialTeleport.Position)) -- Dịch chuyển thẳng tới Part cổng
                        end
                    end
                end
            end)
...
-- Hàm bổ trợ quét thuyền của người chơi (nếu source chưa có thì dùng hàm này)
...
local ToggleFindVolcano = Tabs.Sea:AddToggle("ToggleFindVolcano", {
    Title = "Find Volcano Island", 
    Description = "", 
    Default = false
})

ToggleFindVolcano:OnChanged(function(Value)
    _G.Prehis_Find = Value
end)
Options.ToggleFindVolcano:SetValue(false)

-- Vòng lặp xử lý Logic di chuyển/Tìm đảo
    ...
    while wait(1) do
        pcall(function()
            if _G.Prehis_Find then
                ...
                -- Tìm vị trí đảo trong Locations
                local PrehistoricLoc = workspace._WorldOrigin.Locations:FindFirstChild("Prehistoric Island", true) or workspace._WorldOrigin.Locations:FindFirstChild("Volcano", true)
                
                if not PrehistoricLoc then
                    -- Nếu chưa có đảo, kiểm tra xem có thuyền chưa
                    local MyBoat = CheckBoat()
                    if not MyBoat then
                        -- Tọa độ bến thuyền để mua thuyền
                        local BoatPos = CFrame.new(-16927.451, 9.086, 433.864)
                        Tween(BoatPos)
                        
                        -- Nếu đã đến gần bến thuyền thì tự động mua thuyền
                    ...
                    -- Nếu có thuyền rồi nhưng chưa ngồi vào ghế lái
                    if char.Humanoid.Sit == false and MyBoat:FindFirstChild("VehicleSeat") then
                        local SeatPos = MyBoat.VehicleSeat.CFrame * CFrame.new(0, 1, 0)
                        Tween(SeatPos)
                        return
                    ...
                    -- Vị trí đi tìm thuyền/đảo ngoài biển sâu
                    PrehisFindBoat = CFrame.new(-10000000, 31, 37016.25)
                    
                    -- Kiểm tra xem có hàm check quái biển không, nếu có nguy hiểm thì bay lên cao (cao độ 150)
                    local hasEnemies = false
                    if type(CheckEnemiesBoat) == "function" and CheckEnemiesBoat() then hasEnemies = true end
                    if type(CheckTerrorShark) == "function" and CheckTerrorShark() then hasEnemies = true end
                    if type(CheckPirateGrandBrigade) == "function" and CheckPirateGrandBrigade() then hasEnemies = true end
                    ...
                    if hasEnemies then
                        ...
                        Tween(PrehisFindBoat)
                    end
                else
                    -- Nếu đảo đã xuất hiện, xử lý dịch chuyển lên đảo an toàn
                    ...
                    if TeleportPart then
                        ...
                        -- Dự phòng nếu không tìm thấy part dịch chuyển cụ thể thì lấy tâm đảo
                        if PrehistoricLoc:IsA("BasePart") or PrehistoricLoc:IsA("Model") then
                            local CenterPos = PrehistoricLoc:IsA("Model") and PrehistoricLoc:GetPivot().Position or PrehistoricLoc.Position
                            local DirToIsland = (CenterPos - hrp.Position).Unit
...
-- Tạo Toggle trên UI Fluent
local ToggleFindVolcano = Tabs.Sea:AddToggle("ToggleFindVolcano", {
    Title = "Find Volcano Island", 
    Description = "Tự động mua thuyền, lái tìm và dịch chuyển đến đảo", 
    Default = false
})
